...
**Объект:** `acc-holo-dev/mta-market-site`
...
MTA Market уже нельзя считать просто концептом: в репозитории есть monorepo, backend/frontend, Prisma schema, Docker/CI, а также отдельный `mta-market-module` с реальной интеграцией с MTA. При этом публичное описание проекта завышает степень готовности: в README проект помечен как `v1.0.0 / production-ready`, тогда как код содержит dev/simulate paths, незавершённые security-механизмы и существенные расхождения с целевой архитектурой.
...
1. фактическое состояние репозитория `mta-market-site`;
2. отдельный `mta-market-module`;
...
Отдельный `mta-market-module` также имеет реальный integration layer, что является большим плюсом.
