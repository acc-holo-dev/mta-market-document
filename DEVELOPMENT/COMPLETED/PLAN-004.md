...
- `apps/server/src/lib/s3.ts`: `uploadToS3` принимает явный object key
  (`key`), медиа кладётся под `media/<media-name>`.
- `apps/server/src/routes/upload.ts`: единый helper `storeMediaObject()`;
...
- `docker-compose.prod.yml`: добавлены `DRM_MASTER_KEY`,
  ...
  REFRESH=30.
- `.env.example` (root) и `apps/server/.env.example` переписаны как полная
...
### Workstream F — DRM
- `mta-market-module/source/drm/license_client.cpp`: challenge подписывается
...
- `tests_drm/main.cpp`: новый interop-блок — challenge декодируется в 32
...
- `nginx.conf`: Host-инъекция в HTTP-редиректе закрыта (allowlist доменов +
  ...
  восстановлен.
- `apps/server/Dockerfile`: `mkdir -p /app/uploads && chown nodejs:nodejs`
...
- `.gitignore`: убрана строка `apps/server/migrations/` — закоммиченный ref
