...
three release independently. See [architecture.md](architecture.md) for the
runtime picture.

## mta-market-document (canonical documentation)

**Owns:**

- Architecture and domain model (`02-architecture/`).
- Frozen protocol/specification documents: DRM Protocol v2
  ([../03-features/drm/protocol-v2.md](../03-features/drm/protocol-v2.md)),
  artifact format/signing/compatibility ([../03-features/artifacts/](../03-features/artifacts/)),
  payment model ([../03-features/payments/](../03-features/payments/)).
- Security requirements and threat-model linkage (`04-security/`).
- Operations documentation: backup, secrets, release channels (`05-operations/`).
- Current status ([../01-project/status.md](../01-project/status.md)), roadmap,
  compatibility matrix, ADRs, historical reports.
...
|---|---|---|
| Any `lib/drm/protocol.ts` constant | `03-features/drm/protocol-v2.md`, compatibility matrix, module `source/drm/` | protocol compatibility test (cross-repo CI, O-006 — planned) + suites on both sides |
| Contract schema change | `02-architecture/database.md` summary only (schema stays in site) | `db.orm` contract check, server test suite |
| New payment provider | `03-features/payments/overview.md`, `yookassa.md` stays provider-specific | webhook/state-machine tests of the new provider |
| Security-relevant change | `04-security/threat-model-linkage.md` row (implementation + test) | the linked test |
